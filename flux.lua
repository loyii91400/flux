...
-- Modified to support getter and setter functions
--
...
local function makesetter(field, checkfn, errormsg)
	return function(self, x)
		if checkfn and not checkfn(x) then
			error(errormsg:gsub("%$x", tostring(x)), 2)
		end
		self[field] = x
		return self
	end
...
function tween.new(obj, time, vars, getters, setters)
	...
	self._ease = "quadout"
	self.vars = {}
	self.getters = getters or {} -- Store getter functions
	self.setters = setters or {} -- Store setter functions
...
function flux:to(obj, time, vars, getters, setters)
	return flux.add(self, tween.new(obj, time, vars, getters, setters))
end
